...
@Repository
public class CountryRepositoryImplement implements CountryRepository {
    ...
    public Country createOrUpdate(Country country) {
        //ktra neu country null => trả về null
        if (country == null) return null;
        //ktra neu id của country null => tạo mới
        if (country.getId() == null) return create(country);
        //ktra neu id duoc cung cấp nhưng ko tồn tại trong database => trả về null
        if (get(country.getId()) == null) return null;

        //nếu id đã tồn tại và có trong database, cập nhật thông tin
        return save(country);
    }
